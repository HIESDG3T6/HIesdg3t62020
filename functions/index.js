...
exports.sendNotifications = functions.database.ref('/notifications/{notificationId}').onWrite((event) => {
  ...
  const messagebody = string.concat(NOTIFICATION_SNAPSHOT.val().date, NOTIFICATION_SNAPSHOT.val().time);
  const payload = {
    notification: {
      title: `New Appointment from ${NOTIFICATION_SNAPSHOT.val().user}!`,
      body: messagebody,
