...
class DiagnosisClient:
    ...
    def json(self):
        return {"_handleRequiredArguments": self._handleRequiredArguments, "_language": self._language, "_healthServiceUrl": self._healthServiceUrl, "_token": self._token}
    ...
    def loadSymptoms(self):
        symptomList = self._loadFromWebService("symptoms")
        return jsonify({"symptoms": symptomList.json()})
        ...
        class PriaidDiagnosisClientDemo:
            ...
            def _loadBodyLocations(self):
                ...
                for bodyLocation in bodyLocations:
                    result += "{0} ({1})".format(bodyLocation["Name"], bodyLocation["ID"])

            result = self._loadBodyLocations()
            print(result)
