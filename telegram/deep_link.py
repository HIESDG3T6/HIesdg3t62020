...
def start(update, context):
    ...
    bot = context.bot
    url = helpers.create_deep_linked_url(bot.get_me().username, CHECK_THIS_OUT, group=False)
...
def deep_linked_level_1(update, context):
    ...
    bot = context.bot
    url = '127.0.0.1/esd/patient/account.html'
    text = "You have successfully started Telegram notifications! \n" \
           " Click Here to return to account details."
    keyboard = InlineKeyboardMarkup.from_button(
        InlineKeyboardButton(text='Click here!', url=url)
    )
...
chat_id = bot.get_updates()[-1].message.from_user.id
print(chat_id)
